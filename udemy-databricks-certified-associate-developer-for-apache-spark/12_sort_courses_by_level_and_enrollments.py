...
courses_df.sort(
    when(col('level') == 'Beginner',1).otherwise( when(col('level')=='Intermediate', 2).otherwise(3) )
).show()
...
def sort_courses_by_level_and_enrollments(courses_df):
    courses_sorted = courses_df.sort( (when(col('level') == 'Beginner', 1).otherwise( when( col('level')=='Intermediate', 2 ).otherwise(3) ) ).desc(), col('enrolments').desc() )
