...
courses_df.select('suitable_for').filter(col('suitable_for') == 'Beginner' ).\
count()
...
def get_course_count(courses_df, suitable_for):
    course_count = courses_df.select('suitable_for').filter( lower(col('suitable_for')) == lower(lit(suitable_for)) ).count()
    return course_count
